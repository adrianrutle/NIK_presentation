...
\column{.5\textwidth}
  \only<1->{
	\centering
	\includegraphics[scale=0.2]{ex_projects_hierarchy_top}
	}
  \only<2>{
	\centering
  ...
  \only<3->{
	\centering
	\includegraphics[scale=0.2]{top}
}

\column{.5\textwidth}
	\only<3>{
	\begin{itemize}
		\item<3> An RDF document is a set of triples
		$(s,p,o) \in (U)\times U\times(U\cup D)$, where
		...
		\item May view $(s,p,o)$ as source, edge, target (hence as graph)
	\end{itemize}
	}
	\only<4>{
	\begin{block}{Observations}	
	\begin{itemize}
		\item<4> The same element may be both vertex and edge
		\item<4> The set of literals $D$, can only appear
		as a destination node of an arrow
		\item<4> The set of vertices is not explicitly defined
		\item<4> A vertex can only exist if it is connected to some arrow
	\end{itemize}
	\end{block}
	}	
\end{columns}
...
\begin{frame}[t]%{DPF as RDF by example}
  \only<1->{
      \centering
      \includegraphics<1>[scale=0.4]{ex_projects_hierarchy_new}
      \includegraphics<2>[scale=0.4]{ex_projects_hierarchy_new-1}
      \includegraphics<3>[scale=0.4]{ex_projects_hierarchy_new-2}
      \includegraphics<4>[scale=0.4]{ex_projects_hierarchy_new-3}
  }
  \only<2>{
      \centering
